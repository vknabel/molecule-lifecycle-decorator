...
export class PaginationHostComponent<T> {
  ...
  @ViewChild(PaginationDirective) public paginationDirective: PaginationDirective<T>;
}
...
describe('pagination component', () => {
  ...
  it('displays content when stable and having two pages', async(() => {
    host.itemsForPage = ((page, forceReload) => page === 0 ? Observable.of([1, 2, 3, 4]) : Observable.of([1]));
    hostFixture.detectChanges();
    hostFixture.whenStable().then(() => {
      expect(hostFixture.debugElement.queryAll(By.css('[testContent]')).length).toEqual(4);
    });
  }));
  ...
  it('displays content when stable and having two pages', async(() => {
    ...
    hostFixture.whenStable().then(() => {
      hostFixture.detectChanges();
      expect(hostFixture.debugElement.queryAll(By.css('[testContent]')).length).toEqual(5);
    });
  }));
  ...
  it('displays content when stable and having one page', async(() => {
    ...
    hostFixture.detectChanges();
    hostFixture.whenStable().then(() => {
      expect(hostFixture.debugElement.queryAll(By.css('[testContent]')).length).toEqual(3);
    });
  }));
  ...
  it('displays empty when stable and empty', async(() => {
    ...
    hostFixture.detectChanges();
    hostFixture.whenStable().then(() => {
      expect(hostFixture.debugElement.queryAll(By.css('[testEmpty]')).length).toEqual(1);
    });
  }));
  ...
  it('displays loading when never emitted', async(() => {
    ...
    hostFixture.detectChanges();
    hostFixture.whenStable().then(() => {
      expect(hostFixture.debugElement.queryAll(By.css('[testLoading]')).length).toEqual(1);
    });
  }));
